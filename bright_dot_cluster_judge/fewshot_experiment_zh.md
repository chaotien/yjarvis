...
3. **這一對 patch 必須同位置、同尺寸、已對齊。** 「想像把 ref 從 def 減掉」的前提是兩張已
   co-register。沒對齊的話,模型再強也算不出有意義的差異。編碼前先對齊。
...
- **`missClu%`:** 真 cluster 被判成 single 的比例 —— **漏報**(把真正的成團/缺陷低估成良性單點)。
- **`missSgl%`:** 真 single 被判成 cluster 的比例 —— **過度報警**(把良性單點升級成 cluster)。
...
> **哪個漏判更危險是你決定的**:缺陷嚴重度的流程通常 `missClu%`(漏報)最危險;但若 cluster
> 會觸發昂貴的重工,`missSgl%`(假警報)可能更要命。腳本兩個都印 —— **看表前先定好優先序**。
