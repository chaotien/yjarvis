...
## 1.5 領域知識 — 把「找角」轉成「找兩個 pattern 邊界的交點」

`system_prompt()` 在 `_TASK_RULES` 之前注入了一段 `_DOMAIN_KNOWLEDGE`,把任務的推理路徑
明確化為四步驟(完整文字見 `run_corner_locate_experiment.py`):

1. 確認 ROI 右上方有 area(看得到重複 pattern);
2. 沿 area 向左下追蹤,**重複 pattern 中斷的最左欄** → `X_left`;
3. 同樣追蹤,**重複 pattern 中斷的最下列** → `Y_bottom`;
4. 輸出 `(X_left, Y_bottom)` 作為 `(x, y)`。

### 為什麼對「座標輸出」任務尤其重要

座標任務本來最大的痛點是:**「角」這個概念對 VLM 是 open-ended**,
模型要自己決定「角長什麼樣、在哪、座標怎麼算」。每一步都會引入誤差。
領域知識把這個 open-ended 問題拆成三個更具體的子問題:

| 沒有領域知識 | 加入領域知識 |
|---|---|
| 「找角的座標」 | 「找重複 pattern 中斷的最左欄 X_left」 |
| | 「找重複 pattern 中斷的最下列 Y_bottom」 |
| | 「輸出 (X_left, Y_bottom)」 |

每個子問題對 VLM 都比原問題具體 → 推理出來的座標也更可預期。

### 跟 few-shot 怎麼互動

- 沒有領域知識時:few-shot 只能教模型「答案長什麼樣」(座標的格式、範圍)。
- 加上領域知識後:few-shot 還可以教模型「**推理路徑**長什麼樣」。
  `_exemplar_answer()` 在 manifest 沒提供 reasoning 時,fallback 文字也照著
  「area → X_left → Y_bottom → 交點」的順序合成,讓 assistant turn 同時展示
  答案與推理。`raw_results.csv` 裡的 `raw` 欄位可以看到模型有沒有照學。

### A/B 比較怎麼做

把 `system_prompt()` 裡 `+ _DOMAIN_KNOWLEDGE +` 拿掉,跑完整 ladder,比對 `hit@5%`、
`wrongQ%`、`medianL2%`。如果領域知識真的幫忙,差距會在 reasoning 條件下最明顯
(因為它就是給推理用的)。
